...
def compute_fraes_advantages(rewards, values, dones, gamma=0.99, alpha=0.01, re_crit=2.0, beta_laminar=0.95, eps=1e-8):
    ...
    for t in range(T):
        ...
        # Navier-Stokes Phase Transition Switch
        if re_info < re_crit:
            betas[t] = beta_laminar  # Laminar Flow: Information flows forward
        else:
            betas[t] = 0.0           # Turbulent Flow: Instant Truncation to protect policy
